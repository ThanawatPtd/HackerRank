...
func kangaroo(x1 int32, v1 int32, x2 int32, v2 int32) string {
	...
	if v1 == v2 { // has the same jump range
		//has the same position
		if x1 == x2 {
			return "YES"
		}
		// position is not same
		return "NO"
	}

	// the starting point of kangaroo is lower and jump range is also lower then they will never meet
	if (x1 < x2 && v1 <= v2) || (x2 < x1 && v2 <= v1) {
		return "NO"
	}

	// use mathemethic x1 + (n * v1) = x2 + (n * v2)
	if (x2-x1)%(v1-v2) == 0 {
		return "YES"
	}
