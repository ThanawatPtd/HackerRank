...
func getMoneySpent(keyboards []int32, drives []int32, b int32) int32 {
	// Sort keyboards in ascending order and drives in descending order
	...
	maximumPaid := int32(-1)
	i, j := 0, 0 // Pointers for keyboards and drives

	// Use two-pointer approach
	for i < len(keyboards) && j < len(drives) {
		totalCost := keyboards[i] + drives[j]

		if totalCost > b {
			// If the cost exceeds the budget, move the drive pointer to the left
			j++
		} else {
			// If the cost is valid, update the maximum cost and move the keyboard pointer
			if totalCost > maximumPaid {
				maximumPaid = totalCost
			}
			i++
		}
	}
