...
func birthdayCakeCandles(candles []int32) int32 {
	var maxHeightCandle int32 = 0
	var numberHeightCandle map[int32]int32 = make(map[int32]int32)
	for _, x := range candles {
		if maxHeightCandle < x {
			maxHeightCandle = x
		}
		numberHeightCandle[x] += 1
	}
	return numberHeightCandle[maxHeightCandle]
}
